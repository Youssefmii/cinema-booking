          ...
          <tbody>
            {movies.map(m => (
              <tr key={m.id} className="border-b border-white/10 hover:bg-white/10">
                ...
                <td className="px-4 py-3 text-slate-400">{m.genre}</td>
                <td className="px-4 py-3 text-slate-400">{m.duration} min</td>
                <td className="px-4 py-3">
                  <button onClick={() => toggle(m)} className={`px-2 py-0.5 rounded-full text-xs font-semibold ${m.is_active ? 'bg-green-500/20 text-green-300' : 'bg-slate-100 text-slate-500'}`}>
                    {m.is_active ? 'Active' : 'Hidden'}
                  </button>
                </td>
                <td className="px-4 py-3">
                  <div className="flex gap-2">
                    <button onClick={() => openEdit(m)} className="p-1.5 text-slate-500 hover:text-blue-600 hover:bg-blue-500/10 rounded-lg"><Edit2 size={16}/></button>
