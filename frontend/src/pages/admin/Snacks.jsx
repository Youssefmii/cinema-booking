  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-white">Snack Menu</h1>
      ...
      {CATEGORIES.map(cat => grouped[cat].length > 0 && (
        <div key={cat} className="mb-6">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {grouped[cat].map(s => (
              <div key={s.id} className="bg-white/10 rounded-xl border border-white/15 p-4 flex items-center justify-between">
                <div>
                  <div className="font-medium text-white">{s.name}</div>
                  <div className="text-blue-600 font-semibold text-sm">${s.price.toFixed(2)}</div>
                  <button onClick={() => toggle(s)} className={`mt-1 px-2 py-0.5 rounded-full text-xs font-semibold ${s.is_available ? 'bg-green-500/20 text-green-300' : 'bg-slate-100 text-slate-500'}`}>
