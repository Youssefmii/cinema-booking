  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-slate-800">Cinema Halls</h1>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div key={h.id} className="bg-white rounded-xl border border-slate-200 p-5">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div className="bg-slate-50 rounded-lg p-2 text-center">
                <div className="font-bold text-slate-800">{h.rows}</div>
                <div className="text-slate-500 text-xs">Rows</div>
              </div>
              <div className="bg-slate-50 rounded-lg p-2 text-center">
                <div className="font-bold text-slate-800">{h.seats_per_row}</div>
        ...
        {halls.length === 0 && <div className="col-span-3 text-center py-12 text-slate-400 bg-white rounded-xl border border-slate-200">No halls yet</div>}
      </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-xl">
            <div className="flex items-center justify-between p-5 border-b border-slate-200">
              <h2 className="font-bold text-slate-800">{modal === 'add' ? 'Add Hall' : 'Edit Hall'}</h2>
            ...
            <form onSubmit={save} className="p-5 space-y-4">
              <div>
                ...
                <input required value={form.name} onChange={e => setForm({...form, name: e.target.value})}
                  className="w-full px-3 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" placeholder="e.g. Hall 1"/>
              </div>
              {modal === 'add' && (
                <>
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Number of Rows</label>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Seats per Row</label>
